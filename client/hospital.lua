...
-----------------------------ADDED PED BY MIKEY FLASH
CreateThread(function()
    local model = 's_m_m_doctor_01' -- Scrubs guy
    RequestModel(model)
    while not HasModelLoaded(model) do
        Wait(100)
    end

    local ped = CreatePed(0, model, 308.35, -595.47, 43.28 - 1.0, 71.12, false, true)
    SetEntityHeading(ped, 71.12)
    ...
    TaskPlayAnim(ped, 'missheistdockssetup1clipboard@base', 'base', 8.0, -8, -1, 1, 0, false, false, false)

    -- Attach clipboard prop
    local clipboard = CreateObject(GetHashKey('prop_notepad_01'), 0, 0, 0, true, true, true)
    AttachEntityToEntity(clipboard, ped, GetPedBoneIndex(ped, 18905), 0.1, 0.02, 0.05, 10.0, 0.0, 0.0, true, true, false, true, 1, true)
end)
